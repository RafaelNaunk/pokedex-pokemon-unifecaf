# pokedex-pokemon-unifecaf


‘
Aplicação Front-End para uma API Rafael Naunnccik Web Programming For Front End - UNIFECAF
1.
Introdução
2.
O que é uma API e consumo de dados em tempo real
3.
Manipulação do DOM e criação de elementos dinâmicos
4.
Funções JavaScript utilizadas no projeto
5.
Escolha da PokeAPI
6.
Regras de acesso e documentação da PokeAPI
1) Introdução
...
1)
Pokémon é uma franquia de grande sucesso dos anos 90 e 2000;
2)
A documentação da PokeAPI é clara e de fácil compreensão para iniciantes;
3)
A API não exige autenticação por token, o que simplifica o processo de consumo dos dados no contexto de um projeto acadêmico.
6) Regras de acesso e documentação da PokeAPI
A PokeAPI é uma API pública e gratuita, que expõe seus recursos a partir da URL base https://pokeapi.co/api/v2/. No projeto foi utilizado principalmente o endpoint /pokemon/{id ou nome}, que permite consultar um Pokémon específico informando seu número ou nome. A resposta retorna um objeto JSON contendo campos como id, name, sprites (imagens), types (tipos do Pokémon), height (altura) e weight (peso), entre outros.
